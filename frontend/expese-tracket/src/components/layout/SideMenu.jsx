  ...
  return (
    <div className='w-64 h-[calc(100vh-64px)] bg-white  border-r border-gray-200/50 sticky top-[64px] z-20'>
      <div className='flex flex-col items-center justify-center gap-3 mt-3 mb-7'>
        {
          user?.profileImage ? (
            <img src={user?.profileImage||""} className='w-20 h-20 bg-slate-400 rounded-full' alt="profile image" />
      ...
      <h5 className='text-gray-900 font-medium leading-6 mb-4'>{user?.fullName || ""}</h5>
      </div>
      {
        side_Menu_Data.map((item, index) => (
          <button key={`menu_${index}`} className={`w-full flex items-center gap-4 text-[15px] cursor-pointer ${activeMenu ==item?.label ?"bg-purple-500 text-white" : ""} py-3 px-6 rounded-lg mb-3 `} onClick={() => handleClick(item.path)}>
